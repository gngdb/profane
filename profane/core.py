...
# %% auto 0
__all__ = ['logger', 'RCDIR', 'get_rcdir', 'parse_args', 'setup', 'write_profanerc', 'get_config', 'persistent_scan_record',
           'iterate_records', 'history_csv', 'parse_output_log', 'parse_stats', 'create_dirs', 'SyncLocalCallback',
           'SyncSharedCallback', 'system_to_string', 'git_patch_callback', 'git_status', 'git_branch', 'init']
...
logging.basicConfig(format='%(asctime)s %(message)s %(name)s', level=logging.ERROR)
...
def persistent_scan_record(ds):
    """This isn't great, but I don't know how to parse profobufs records properly."""
    retries = 0
    while True:
        try:
            return ds.scan_record()
        except (IndexError, AssertionError) as e:
            # format e to str for logging
...
def iterate_records(data_path):
    ...
    data = persistent_scan_record(ds)
